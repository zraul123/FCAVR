...
namespace Ganter.Algorithm
{
    ...
    public class FormalContext
    {
        ...
        public FormalContext(IEnumerable<string> attributes, IEnumerable<string> items, bool[,] matrix)
        {
            Attributes = attributes.Select(x => new Attribute { Name = x }).ToList();
            Items = items.Select(x => new Item { Name = x }).ToList();
            Matrix = matrix;
